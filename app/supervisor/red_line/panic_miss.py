"""HRL-04 · UI panic 未 100ms 响应 detector。

context['panic_latency_report']:
...
@dataclass
class PanicLatencyMissDetector:
    name: str = "panic_miss"
    red_line_id: RedLineId = RedLineId.HRL_04_PANIC_MISS
    ...
    async def detect(
        ...
        report = context.get("panic_latency_report") or {}
        ...
        obs_refs: tuple[str, ...] = tuple(refs[:10]) if refs else (
            f"panic-latency-max-{max_lat}ms",
        )
        hit = RedLineHit(
            red_line_id=self.red_line_id,
            ...
            reason=(
                f"HRL-04: UI panic {len(violations)}/{len(samples)} samples > {thr}ms "
                f"(max {max_lat}ms)"
            ),
