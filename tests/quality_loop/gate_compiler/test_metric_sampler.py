"""L1-04 · L2-04 · MetricSampler tests · IC-L2-04 metric 输入规范化.
...
职责:
- 接收外部 S4/S5 送入的 dict[str, Any] metric · 规范化为 `MetricSample`
- sample_hash 稳定计算（幂等 key）
- project_id 必填（PM-14）
- 类型限制: int / float / bool / str / None（禁嵌套 · 防表达式注入）
"""
...
class TestMetricSamplerHappyPath:
    def test_TC_L204_MS_001_sample_from_dict_basic(self) -> None:
        """TC-L204-MS-001 · 基础 metric dict → MetricSample · hash 非空。"""
        sampler = MetricSampler()
        sample = sampler.sample(
            project_id="p1",
            metrics={"coverage": 0.85, "tests_passed": 12},
            wp_id="wp-01",
        )
        assert isinstance(sample, MetricSample)
        assert sample.project_id == "p1"
        assert sample.wp_id == "wp-01"
        assert sample.values == {"coverage": 0.85, "tests_passed": 12}
    ...
    def test_TC_L204_MS_003_sample_hash_differs_on_value_change(self) -> None:
        ...
        sampler = MetricSampler()
        s1 = sampler.sample(project_id="p1", metrics={"coverage": 0.8})
        s2 = sampler.sample(project_id="p1", metrics={"coverage": 0.9})
    ...
    def test_TC_L204_MS_004_sample_hash_includes_project_id(self) -> None:
        ...
        sampler = MetricSampler()
        s1 = sampler.sample(project_id="p1", metrics={"a": 1})
        s2 = sampler.sample(project_id="p2", metrics={"a": 1})
        assert s1.sample_hash != s2.sample_hash

    def test_TC_L204_MS_005_accepts_bool_none_str(self) -> None:
        """TC-L204-MS-005 · bool / None / str 全通过。"""
        sampler = MetricSampler()
        sample = sampler.sample(
            project_id="p1",
            metrics={
        ...
        assert sample.values["flag"] is True
        assert sample.values["maybe"] is None
        assert sample.values["status"] == "green"
...
class TestMetricSamplerValidation:
    def test_TC_L204_MS_010_empty_project_id_raises(self) -> None:
        ...
        with pytest.raises(MetricSamplerError, match="E_L204_NO_PROJECT_ID"):
            sampler.sample(project_id="", metrics={"a": 1})
    ...
    def test_TC_L204_MS_012_nested_dict_rejected(self) -> None:
        """TC-L204-MS-012 · 嵌套 dict · E_L204_MS_NESTED_VALUE。"""
        sampler = MetricSampler()
        with pytest.raises(MetricSamplerError, match="E_L204_MS_NESTED_VALUE"):
            sampler.sample(project_id="p1", metrics={"nested": {"k": 1}})

    def test_TC_L204_MS_013_list_value_rejected(self) -> None:
        """TC-L204-MS-013 · list 值 · E_L204_MS_NESTED_VALUE。"""
        sampler = MetricSampler()
        with pytest.raises(MetricSamplerError, match="E_L204_MS_NESTED_VALUE"):
            sampler.sample(project_id="p1", metrics={"arr": [1, 2, 3]})

    def test_TC_L204_MS_014_non_identifier_key_rejected(self) -> None:
        """TC-L204-MS-014 · metric key 非 identifier · E_L204_MS_BAD_KEY."""
        sampler = MetricSampler()
        with pytest.raises(MetricSamplerError, match="E_L204_MS_BAD_KEY"):
            sampler.sample(project_id="p1", metrics={"bad-key!": 1})
...
class TestMetricSampleVO:
    def test_TC_L204_MS_020_sample_is_frozen(self) -> None:
        ...
        sampler = MetricSampler()
        sample = sampler.sample(project_id="p1", metrics={"a": 1})
