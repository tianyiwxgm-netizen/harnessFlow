...
| 4 | 时序图形式 | **PlantUML**（v1.2 update · 原 Mermaid 改为 PlantUML）| 用户 IDE（IntelliJ Markdown 插件）已启用 PlantUML 内联渲染；PlantUML 比 Mermaid 更成熟，支持 Component / Deployment / State / Activity 完整 UML 类型；图形美观、可导出 SVG/PNG；所有已有 Mermaid 块需转换为 PlantUML |
...
### 6.4 PlantUML 图表规范（v1.2 update · 原 Mermaid 改为 PlantUML）

> **强制约定**：本项目所有 3-Solution 文档的图表**一律使用 PlantUML**（sequenceDiagram / componentDiagram / stateDiagram / activityDiagram / classDiagram / deploymentDiagram 全套）。理由：用户 IDE（IntelliJ Markdown 插件 + PlantUML 插件）已启用内联渲染；PlantUML 图形美观、类型完整、可导出 SVG/PNG。
>
> **已有 Mermaid 代码块必须转换**（详见 Phase 3-pre `3-Sol-P3.5` 迁移任务）。

每份 tech-design 至少 1 张 PlantUML 时序图（P0 或关键流程）。使用 `plantuml` code block（markdown 围栏语法），且务必包含 `@startuml ... @enduml` 界符（IntelliJ PlantUML 插件识别必需）：

**时序图模板**：

```plantuml
@startuml
title IC-01 状态转换请求（P0）
participant "L1-01 主 loop" as L1_01
participant "L1-02 生命周期" as L1_02
participant "L1-09 事件总线" as L1_09

L1_01 -> L1_02: IC-01 request_state_transition\n{from, to, reason}
activate L1_02
L1_02 -> L1_02: 校验 allowed_next
L1_02 --> L1_01: {accepted: true, new_entry}
deactivate L1_02
L1_01 -> L1_09: publish(L1-01:state_changed)
@enduml
```

**状态图模板**（替代 Mermaid stateDiagram-v2）：

```plantuml
@startuml
[*] --> S0_init
S0_init --> S1_plan : user_approve_scope
S1_plan --> S2_split : plan_approved
S2_split --> S3_design : wbs_ready
S3_design --> S4_execute : design_approved
S4_execute --> S5_verify : wp_complete
S5_verify --> S6_wrap : dod_pass
S5_verify --> S4_execute : dod_fail[重做]
S6_wrap --> [*]
@enduml
```

**组件图模板**（替代 Mermaid flowchart）：
...
package "L1-01 主 Agent 决策循环" {
  ...
  component "L2-04 任务链执行器" as L2_04
  component "L2-05 决策审计记录器" as L2_05
  component "L2-06 Supervisor 接收器" as L2_06
}

L2_01 --> L2_02 : on_tick (IC-L2-01)
L2_02 --> L2_03 : request_state_transition (IC-L2-02)
L2_02 --> L2_04 : start_chain (IC-L2-03)
L2_02 --> L2_05 : decision_made (IC-L2-05)
L2_06 ..> L2_02 : inject_warn (内存入队)
L2_06 ..> L2_01 : async_cancel (BLOCK)
@enduml
```

**禁止**：
- ❌ `\`\`\`mermaid` 代码块（一律改 `\`\`\`plantuml`）
- ❌ 无 `@startuml/@enduml` 界符的纯 `{ ... }` 语法
- ❌ Mermaid 语法混入 PlantUML 块（`->>` vs `->`；`participant X as "Y"` vs `participant "Y" as X`）
