...
- 前置：Infra-0 done（`app/project_lifecycle/common/` + `templates/` 空目录）。
...
```
app/project_lifecycle/template_engine/
...
- [ ] **A1**：扩 `tests/project_lifecycle/conftest.py` · 加 WP01 fixture

追加以下 fixture（保持 Infra-0 已有的 `mock_project_id` / `mock_request_id` / `mock_event_bus` 不动）：

```python
# tests/project_lifecycle/conftest.py 追加部分
...
- [ ] **A2**：建 `app/project_lifecycle/template_engine/errors.py`
...
- [ ] **A3**：建 `app/project_lifecycle/template_engine/schemas.py`
...
```bash
git add app/project_lifecycle/template_engine/errors.py \
        app/project_lifecycle/template_engine/schemas.py \
        app/project_lifecycle/template_engine/__init__.py \
        tests/project_lifecycle/conftest.py
...
- [ ] **C1**：写 `tests/project_lifecycle/test_l2_07_template_engine_startup.py`（红）
...
```bash
pytest tests/project_lifecycle/test_l2_07_template_engine_startup.py -v
```

预期：`ModuleNotFoundError: app.project_lifecycle.template_engine.registry` 或 AttributeError。

- [ ] **C3**：写 `app/project_lifecycle/template_engine/sandbox.py`（绿前半）
...
- [ ] **C4**：写 `app/project_lifecycle/template_engine/registry.py`（绿核心）
...
- 检查各 frontmatter 合法（运行 `python -c "from app.project_lifecycle.template_engine.registry import TemplateLoader; TemplateLoader('templates').load_all()"`）

- [ ] **C6**：commit

```bash
git add app/project_lifecycle/template_engine/sandbox.py \
        app/project_lifecycle/template_engine/registry.py \
        tests/project_lifecycle/test_l2_07_template_engine_startup.py
...
- [ ] **D1**：写 `app/project_lifecycle/template_engine/hashing.py`（绿前件）
...
- [ ] **D2**：写 `tests/project_lifecycle/test_l2_07_template_engine_positive.py`（红）

详见 TDD md `§2` 中 TC-001~013 全文。复制到 `tests/project_lifecycle/test_l2_07_template_engine_positive.py`（用之前已读的 §2 代码块为准 · 全 14 TC 含 013 的 validate_slots fail 场景）。

**重点代码片段**（完整内容太长 · 请从 `docs/3-2-Solution-TDD/L1-02-项目生命周期编排/L2-07-产出物模板引擎-tests.md §2` 复制全部）：

```python
# tests/project_lifecycle/test_l2_07_template_engine_positive.py
from __future__ import annotations
import hashlib
from typing import Any
...
（实际执行步骤：`sed -n '108,396p' docs/3-2-Solution-TDD/L1-02-项目生命周期编排/L2-07-产出物模板引擎-tests.md` 取 §2 代码 → 粘贴覆盖文件 · 修 `from app.l2_07...` → `from app.project_lifecycle.template_engine...`）

- [ ] **D3**：跑红

```bash
pytest tests/project_lifecycle/test_l2_07_template_engine_positive.py -v
```

预期：全红（TemplateEngine 尚未建）。

- [ ] **D4**：写 `app/project_lifecycle/template_engine/renderer.py`（绿核心 1/2）
...
from app.project_lifecycle.template_engine.registry import TemplateRegistry, TemplateEntry
from app.project_lifecycle.template_engine.schemas import RenderedOutput
...
- [ ] **D5**：写 `app/project_lifecycle/template_engine/engine.py`（绿核心 2/2）
...
from app.project_lifecycle.template_engine.registry import TemplateLoader, TemplateRegistry
from app.project_lifecycle.template_engine.renderer import render_core, ENGINE_VERSION
from app.project_lifecycle.template_engine.schemas import RenderedOutput, ValidationResult
...
class TemplateEngine:
    ...
    def load_from_dir(
        ...
        from app.project_lifecycle.template_engine.registry import REQUIRED_KINDS_DEFAULT
        loader = TemplateLoader(
...
- [ ] **D6**：补 `app/project_lifecycle/template_engine/__init__.py` 的 export

```python
"""L2-07 产出物模板引擎 · public API。"""
from app.project_lifecycle.template_engine.engine import TemplateEngine
from app.project_lifecycle.template_engine.schemas import RenderedOutput, ValidationResult, TemplateEntry
from app.project_lifecycle.template_engine.errors import TemplateEngineError, StartupError
...
```bash
git add app/project_lifecycle/template_engine/{hashing,renderer,engine,__init__}.py \
        tests/project_lifecycle/test_l2_07_template_engine_positive.py
...
- [ ] **E1**：写 `tests/project_lifecycle/test_l2_07_template_engine_negative.py`

复制 `docs/3-2-Solution-TDD/.../L2-07-产出物模板引擎-tests.md §3` 代码（TC-101~115）· 修 import 为 `app.project_lifecycle.template_engine.*`。
...
```bash
pytest tests/project_lifecycle/test_l2_07_template_engine_negative.py -v
```
...
```bash
git add tests/project_lifecycle/test_l2_07_template_engine_negative.py \
        tests/project_lifecycle/conftest.py \
        app/project_lifecycle/template_engine/engine.py
...
对应 `tests/project_lifecycle/test_l2_07_ic_contracts.py` · 复制 3-2 TDD md §4。

- [ ] **F1**：写测试文件（从 TDD md §4 复制 · 共 7 TC）
- [ ] **F2**：跑绿

```bash
pytest tests/project_lifecycle/test_l2_07_ic_contracts.py -v
```

- [ ] **F3**：commit

```bash
git add tests/project_lifecycle/test_l2_07_ic_contracts.py
...
- [ ] **G1**：写 `tests/project_lifecycle/test_l2_07_perf.py` · 从 TDD md §5 复制
- [ ] **G2**：跑

```bash
pytest tests/project_lifecycle/test_l2_07_perf.py -v --durations=10
```
...
```bash
git add tests/project_lifecycle/test_l2_07_perf.py
...
```bash
pytest tests/project_lifecycle/test_l2_07_*.py --cov=app/project_lifecycle/template_engine --cov-report=term-missing -v
```

预期：≥ 56 TC 全绿 · coverage ≥ 85%（app/project_lifecycle/template_engine/）。

- [ ] **H2**：mypy

```bash
mypy app/project_lifecycle/template_engine/
```
...
```bash
ruff check app/project_lifecycle/template_engine/ tests/project_lifecycle/
```

预期：clean。

- [ ] **H4**：写 `app/project_lifecycle/template_engine/README.md`（~60 行）
...
```bash
git add app/project_lifecycle/template_engine/README.md
...
- [ ] coverage ≥ 85%（`app/project_lifecycle/template_engine/`）
...
- [ ] `app/project_lifecycle/template_engine/README.md` 就位
...
- **情形 D · TDD 契约不一致**：TDD md §2 中用了 `app.l2_07.engine` 而 tech-design 无明确路径 · 本 plan 按 `app.project_lifecycle.template_engine.engine` 统一 · 若集成时 Dev-β/γ 引用路径不同，走 `_correction_log.jsonl` 仲裁。
