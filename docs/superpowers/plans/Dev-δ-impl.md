...
**Architecture:** 7 L2 都在 `app/project_lifecycle/` 下，按职责分子包；L2-07 模板引擎（Jinja2 SandboxedEnvironment + jsonschema）是横切地基，先实现；L2-02 起点（PM-14 pid 创建入口）+ L2-01 Gate 控制器（IC-01 入口 + 硬禁自动放行）为控制骨架；L2-03/04/05 是产出器（4 件套 + PMP 9 + TOGAF 9 Phase）；L2-06 是收尾（tar.zst + PM-14 归档入口）。所有 L2 共用 `app/project_lifecycle/common/` 的 schema / errors / event 发射工具。

**Tech Stack:** Python 3.11+ · Jinja2 sandboxed (>=3.1) · jsonschema (Draft 2020-12) · ulid-py · pydantic v2 · PyYAML · zstandard (tar.zst) · asyncio.gather（并发） · pytest + pytest-asyncio + pytest-benchmark · mypy 严模式（`app.project_lifecycle` 加 packages）。
...
|:---:|:---|:---|:---:|:---|:---:|:---:|
| Infra-0 | `Dev-δ-infra-0.md` | pyproject + app/project_lifecycle 骨架 + templates/ 目录 | 📝 待写 | — | 0.25 天 | — |
...
├── integration/
│   ├── __init__.py
│   ├── test_project_lifecycle_e2e.py                   # S1→S7 mock 全链 ≥15 TC
...
- `[tool.coverage.run].source` 加 `"app/project_lifecycle"`
- `[tool.mypy].packages` 加 `"app.project_lifecycle"`
...
- 全域 base：`app/project_lifecycle/common/errors.py::L102Error`（带 `error_code`、`context`、`caller_l2`、`project_id` 字段）

### 3.2 事件发射（IC-09 mock）

- 接口：`app/project_lifecycle/common/event_emitter.py::EventEmitter`
...
- pid 格式：ULID（26 char · lexicographic sortable · `app/project_lifecycle/common/pid_validator.py` 校验）
...
- 每 WP 结束：`pytest tests/project_lifecycle/test_l2_0N_*` 全绿 + coverage ≥ 85%
...
```toml
[tool.coverage.run]
source = ["app/l1_09", "app/project_lifecycle"]   # 扩
branch = true

[tool.mypy]
packages = ["app.l1_09", "app.project_lifecycle"]  # 扩
```
...
### Infra-0.2 建 app/project_lifecycle/ 骨架

**Files:**
- Create: `app/__init__.py`（若已存在则跳过）
- Create: `app/project_lifecycle/__init__.py`
- Create: `app/project_lifecycle/README.md`
- Create: `app/project_lifecycle/common/{__init__,errors,event_emitter,pid_validator,schemas}.py`
- Create: `app/project_lifecycle/template_engine/__init__.py`
- Create: `app/project_lifecycle/kickoff/__init__.py`
- Create: `app/project_lifecycle/four_set/__init__.py`
- Create: `app/project_lifecycle/stage_gate/__init__.py`
- Create: `app/project_lifecycle/pmp/__init__.py`
- Create: `app/project_lifecycle/togaf/__init__.py`
- Create: `app/project_lifecycle/closing/__init__.py`
- Create: `tests/__init__.py`（若已存在则跳过）
- Create: `tests/project_lifecycle/__init__.py`
- Create: `tests/project_lifecycle/conftest.py`

- [ ] **Step 1**：建空子包 + 占位 docstring

每个 `__init__.py` 仅 2-3 行 docstring 标明职责。`app/project_lifecycle/README.md` 列出 7 L2 + common 的分工（复用 Dev-δ md §1.2）。
...
- [ ] **Step 5**：`tests/project_lifecycle/conftest.py` 写共享 fixture
...
python -c "from app.project_lifecycle.common.errors import L102Error; from app.project_lifecycle.common.event_emitter import EventEmitter; from app.project_lifecycle.common.pid_validator import is_valid_pid; print('import OK')"
pytest tests/project_lifecycle/ -v --collect-only | head -5
```
...
```bash
git add pyproject.toml app/project_lifecycle tests/project_lifecycle
...
5. **DoD 自检**：引 Dev-δ md §3.X DoD + 加 `pytest tests/project_lifecycle/test_l2_0N_*.py -v` 全绿 + coverage
...
- [ ] `app/project_lifecycle/common/` import 通过（Infra-0 done）
...
- `pytest tests/project_lifecycle/ -v` 看当前绿的 TC
