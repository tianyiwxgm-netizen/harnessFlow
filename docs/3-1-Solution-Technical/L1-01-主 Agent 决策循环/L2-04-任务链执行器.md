...
任务链执行器核心职责是 DAG 调度、步骤执行编排、回滚协调。测试替身需同时支持：

| 模式 | 适用测试 | Mock 深度 | DAG/回滚行为 |
|---|---|---|---|
| **unit（单元）** | pytest 单元，步骤结果预设 | 全替身，不走真实拓扑排序 | DAG 顺序固定；回滚即刻生效 |
| **contract（契约）** | CDC 跨 L2（L1-04 WP 执行器）接口 | DAG 真实 + IC schema 校验 | 回滚按 fixture 行为 |
| **chaos（混沌）** | 并发 / 超时 / 部分回滚测试 | 主流程真实 + 步骤级故障注入 | 注入超时 / 回滚失败 / compensation 延迟 |

**模式激活环境变量**：

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `L2_04_MOCK_MODE` | `off` | `unit` / `contract` / `chaos` / `off` |
| `L2_04_MOCK_STEP_DELAY_MS` | 0 | 每步执行延迟（竞态测试用）|
| `L2_04_MOCK_FAIL_STEP` | — | 单元模式：指定失败 step_id |
| `L2_04_MOCK_DAG_FIXTURE` | — | contract 模式：DAG 定义 YAML |
| `L2_04_MOCK_STEP_TIMEOUT_RATE` | 0.0 | chaos 模式：步骤超时比例 |
| `L2_04_MOCK_ROLLBACK_FAIL_RATE` | 0.0 | chaos 模式：回滚失败比例 |
| `L2_04_MOCK_COMPENSATION_DELAY_MS` | 0 | chaos 模式：补偿事务延迟 |
| `L2_04_MOCK_DETERMINISM_SEED` | 42 | 随机 seed（复现用）|
...
class MockChainExecutor:
    """
    任务链执行器测试替身。支持三种模式：
      1. step_results（unit）：预设步骤结果
      2. dag_fixture（contract）：真实 DAG + IC schema 校验
      3. chaos_config（chaos）：注入超时 / 回滚失败 / 补偿延迟
    """
    def __init__(
        self,
        mode: str = "unit",
        step_results: dict[str, dict] | None = None,
        dag_fixture: str | None = None,
        ...
        self._aborted = False
        self._rollback_trace: list[str] = []
        self._compensation_queue: list[dict] = []
        self._dag = self._load_dag(dag_fixture) if dag_fixture else None
    ...
    def execute_step(self, step_id: str, project_id: str) -> dict:
        # chaos 模式：按 rate 注入超时
        if self._mode == "chaos" and self._rng.random() < self._chaos.get("step_timeout_rate", 0.0):
            return {"status": "TIMED_OUT", "step_id": step_id, "project_id": project_id,
                    "injected": "chaos_timeout"}
    ...
    def rollback_step(self, step_id: str, project_id: str) -> dict:
        # chaos 模式：按 rate 注入回滚失败
        if self._mode == "chaos" and self._rng.random() < self._chaos.get("rollback_fail_rate", 0.0):
            step_type = self._step_results.get(step_id, {}).get("type", "unknown")
            if step_type == "irreversible":  # e.g. 已发出的外部 API
                self._compensation_queue.append({"step_id": step_id, "reason": "irreversible",
                                                 "required_action": "COMPENSATION_NEEDED"})
                return {"status": "COMPENSATION_NEEDED", "step_id": step_id}
            return {"status": "ROLLBACK_FAIL", "step_id": step_id}

        self._rollback_trace.append(step_id)
        return {"status": "ROLLED_BACK", "step_id": step_id, "project_id": project_id}

    def abort_chain(self, chain_id: str, project_id: str) -> dict:
        self._aborted = True
        return {"status": "ABORTED", "chain_id": chain_id, "project_id": project_id,
                "rolled_back": list(self._rollback_trace)}

    def executed_steps(self) -> list[str]:
        return list(self._executed)

    def rollback_trace(self) -> list[str]:
        return list(self._rollback_trace)

    def pending_compensations(self) -> list[dict]:
        return list(self._compensation_queue)
```
...
|---|---|---|---|
| `IC-L2-04.start_chain` | 返回预设 `chain_id`，跳过拓扑排序 | 拓扑排序真实，校验 DAG schema | 10% 启动失败 |
| `IC-L2-04.on_step_completed` | 消费 step_results，触发下一步 | 校验步骤输出 schema | 延迟 100-500ms |
| `IC-L2-04.abort_chain` | 立即返回 `{status: ABORTED}` | 校验 rollback 顺序（LIFO）| 20% 回滚失败 |
| `IC-L2-04.compensate_step` | 标记 `COMPENSATION_NEEDED` | 校验补偿事务 schema | 延迟 1-5s |
| 步骤执行器 IC（L1-04 WP）| `MockStepExecutor`，返回预设 | 校验 WP 入参 schema（IC-02）| 随机失败 + 重试 |
| `IC-01` chain_step_done 事件 | 静默 | 记录 + 校验 schema | 乱序发布 |
| `IC-09` audit 写入 | 静默 | 校验 hash-chain | 注入乱序 |

### §9.4 Fixture 策略

```yaml
# tests/fixtures/l2_04/happy_path_dag.yaml
...
step_results:
  ...
  s4: {status: "DONE", output: {verified: true}}
```

```yaml
# tests/fixtures/l2_04/cycle_detection.yaml
# 循环依赖 fixture（测 SA-01）
project_id: "proj-test-003"
steps:
  - step_id: "s1"
    depends_on: ["s3"]  # 循环：s1→s3→s1
  - step_id: "s2"
    depends_on: ["s1"]
  - step_id: "s3"
    depends_on: ["s2"]
expected_error: "E_CHAIN_CYCLE_DETECTED"
```

```yaml
# tests/fixtures/l2_04/partial_rollback.yaml
# 部分回滚：s3 失败 → 回滚 s2（可逆）+ s1（irreversible → COMPENSATION_NEEDED）
project_id: "proj-test-003"
steps:
  - step_id: "s1"
    type: "irreversible"  # 外部 API
    depends_on: []
  - step_id: "s2"
    type: "reversible"
    depends_on: ["s1"]
  - step_id: "s3"
    ...
    depends_on: ["s2"]
step_results:
  s1: {status: "DONE", output: {external_api_call_id: "ext-001"}}
  s2: {status: "DONE", output: {wp_count: 2}}
  s3: {status: "FAIL", error: "E_STEP_EXEC_FAIL"}
expected_rollback_trace: ["s2"]  # 只回滚 s2
expected_compensations: [{step_id: "s1", required_action: "COMPENSATION_NEEDED"}]
```

### §9.5 使用示例（三层覆盖）

```python
# tests/unit/test_chain_executor_unit.py
def test_happy_path_chain():
    """unit 模式：预设步骤结果"""
    fixture = load_fixture("l2_04/happy_path_dag.yaml")
    mock = MockChainExecutor(mode="unit", step_results=fixture["step_results"])
    for step in fixture["steps"]:
        result = mock.execute_step(step["step_id"], fixture["project_id"])
        assert result["status"] == "DONE"
    assert mock.executed_steps() == ["s1", "s2", "s3", "s4"]

# tests/contract/test_cycle_detection.py
def test_reject_cycle_in_dag():
    """contract 模式：循环依赖检测"""
    fixture = load_fixture("l2_04/cycle_detection.yaml")
    mock = MockChainExecutor(mode="contract", dag_fixture="l2_04/cycle_detection.yaml")
    with pytest.raises(ChainCycleError, match="E_CHAIN_CYCLE_DETECTED"):
        mock.start_chain(fixture["project_id"])

# tests/chaos/test_partial_rollback_compensation.py
def test_irreversible_step_requires_compensation():
    """chaos 模式：不可逆步骤触发补偿事务"""
    fixture = load_fixture("l2_04/partial_rollback.yaml")
    mock = MockChainExecutor(
        mode="chaos",
        step_results=fixture["step_results"],
        chaos_config={"seed": 42, "rollback_fail_rate": 1.0},  # 强制全部"回滚失败"
    )
    # 执行到 s3 失败
    for step in fixture["steps"][:3]:
        mock.execute_step(step["step_id"], fixture["project_id"])
    # 回滚 s2（reversible → ROLLED_BACK）
    mock.rollback_step("s2", fixture["project_id"])
    # 回滚 s1（irreversible → COMPENSATION_NEEDED）
    result = mock.rollback_step("s1", fixture["project_id"])
    assert result["status"] == "COMPENSATION_NEEDED"
    assert len(mock.pending_compensations()) == 1
```
...
任务链执行器威胁面集中在 **DoS**（无限链 / 资源耗尽）、**Tampering**（输出注入）、**Integrity**（回滚越界）：

| # | STRIDE | CIA | 风险点 | Prevent | Detect | Respond | Test Case |
|---|---|---|---|---|---|---|---|
| SA-01 | D | Availability | Chain 定义含循环依赖（A→B→A）| 拓扑排序前 Kahn 检测循环；DAG schema 校验 | 启动时 CI 检查 DAG YAML | `E_CHAIN_CYCLE_DETECTED`，拒启动 | `test_reject_cyclic_dag` / `test_detect_self_loop` |
| SA-02 | T | Integrity | `step_id` 含路径遍历字符 | 正则白名单 `^[a-zA-Z0-9_-]{1,128}$` | Schema 校验阶段拦截 | `E_CHAIN_STEP_ID_INVALID`，拒绝 | `test_reject_traversal_step_id` / `test_reject_injection_step_id` |
| SA-03 | D | Availability | 步骤数量超限（超长链攻击）| 单 chain 最大步骤数 = 500；运行时计数 | Prometheus `l2_04_chain_depth` 告警 | `E_CHAIN_DEPTH_EXCEEDED`，拒启动 | `test_reject_501_step_chain` |
| SA-04 | E | Integrity | 回滚操作越界（rollback 不属于当前 chain）| rollback 入参含 `chain_id`，校验 `step.chain_id == chain_id` | 越界 rollback 100% 审计告警 | `E_CHAIN_ROLLBACK_SCOPE`，拒绝 | `test_reject_cross_chain_rollback` |
| SA-05 | T | Integrity | 步骤输出注入下游 IC payload | 输出 JSON Schema 校验 + 禁 `__proto__` / `constructor` / `prototype` 等危险字段 | DLP 正则扫描 | 拒绝传递，`E_CHAIN_OUTPUT_INJECTION` | `test_reject_prototype_pollution_in_output` |
| SA-06 | R | Auditability | 执行轨迹被删除（chain_id 轨迹丢失）| Append-only JSONL + hash-chain + WORM | 每小时 replay 校验 | 告警 + 从备份恢复 | `test_chain_trace_hash_unbroken` |
| SA-07 | D | Availability | 步骤执行超时无上限 → dispatcher 资源耗尽 | 步骤级 watchdog（默认 30s，可配）；超时 `TIMED_OUT` 标记 | Prometheus `step_timeout_total` 告警 | `E_STEP_TIMEOUT`，触发 rollback | `test_step_timeout_triggers_watchdog` |
| SA-08 | I | Confidentiality | 步骤输出含敏感字段（token / PII）被审计日志记录 | 输出字段白名单 + 敏感字段自动脱敏（`***`）| DLP 日志扫描 | 脱敏 + 回溯影响 | `test_redact_sensitive_output` |
| SA-09 | E | Integrity | 补偿事务（compensation）被越权触发 | compensation 仅由 chain 创建者或 Supervisor 触发（caller_role 校验）| 非法 compensation 100% 审计告警 | `E_CHAIN_COMPENSATION_UNAUTHORIZED`，拒绝 | `test_reject_unauthorized_compensation` |
| SA-10 | T | Integrity | DAG 定义运行时被修改（chain 启动后动态修改 steps）| DAG 启动后不可修改（immutable）；修改尝试抛异常 | 运行时 DAG 修改审计告警 | 进程崩溃 + 告警 | `test_dag_immutable_after_start` |

### §11.2 防御纵深（Defense in Depth）分层

| 层 | 防御措施 | 关联 SA |
|---|---|---|
| **L1 输入校验** | JSON Schema + 正则白名单 + 字段长度限制 | SA-02, SA-05, SA-08 |
| **L2 结构约束** | DAG 循环检测 + 深度限制 + 不可变 | SA-01, SA-03, SA-10 |
| **L3 授权校验** | `caller_role` + chain 归属校验 | SA-04, SA-09 |
| **L4 超时控制** | 步骤级 watchdog + chain 级 timeout | SA-07 |
| **L5 审计不可篡改** | Append-only + hash-chain + WORM | SA-06 |
| **L6 数据边界** | 输出 schema + 敏感字段脱敏 + DLP | SA-05, SA-08 |

### §11.3 审计频率与责任人

- **SA-04, SA-09**：每次 major release 前必重审（Security）
- **SA-01, SA-03, SA-10**：chain 定义变更自动 CI gate（CI 自动）
- **SA-06**：每小时 hash-chain replay（CI 自动）
- **SA-07**：Prometheus 实时监控 step 超时（Oncall）
- **SA-05, SA-08**：DLP 日志扫描（SRE 每日）
...
**问题**：当 Chain 中某步骤超时（watchdog 触发）但步骤本身不报错（步骤进程仍在运行），应等待完成还是强制标记 `TIMED_OUT` 并继续？

**方案选项**：
- **A. 强制 TIMED_OUT + 继续**：按 watchdog 判定，超时即回滚（可能误杀慢任务）
- **B. 延长 grace period**：超时后等待 2× timeout，仍未完成再强制（吃延迟但保留慢任务）
- **C. Supervisor 决策**：超时触发 Supervisor BLOCK，人工/自动决定是否杀（最准确但慢）
- **D. 可配置策略**：按步骤 `type` 配置策略（如 `external_api=wait`, `local_compute=kill`）

**决策维度**：正确性（D>C>B>A）、延迟（A<B<C）、自动化（A>B>D>C）

**关联 IC**：IC-L2-02（决策 replan）、IC-L2-06（Supervisor BLOCK）

**期望决策时间**：M3 前（影响端到端可靠性测试）

---

### OQ-02 · 最大 Chain 深度证据

**问题**：最大 chain 深度 500 是基于实测还是经验值？需要 L1-04 WBS 实际拆解深度数据支撑。

**方案选项**：
- **A. 保守 100**：绝大多数项目够用，但限制复杂项目
- **B. 当前 500**：经验值，缺实测数据
- **C. 实测校准**：在 R3 后基于 L1-04 实际 WBS 产出的 chain 深度 P99 × 2 设定
- **D. 动态限额**：按 project type 差异化（简单项目 100，复杂项目 1000）

**决策维度**：实证强度（C>D>B>A）、运维复杂度（A=B<C<D）、覆盖率（D>C>B>A）

**关联 IC**：L1-04 WBS 产出接口

**期望决策时间**：R3 后（依赖 L1-04 实测数据）

---

### OQ-03 · 补偿事务的不可逆步骤处理

**问题**：对于不可逆步骤（外部 API、已发出的邮件、已提交的交易），标记 `COMPENSATION_NEEDED` 后如何处理？当前仅告警，未设计自动补偿机制。

**方案选项**：
- **A. 人工补偿**：标记后交 Supervisor 人工处理（简单但延迟高）
- **B. 补偿 Saga**：每步骤定义 `compensate()` 函数（如 `send_email` → `send_cancellation_email`）（复杂但自动）
- **C. 两阶段提交**：不可逆步骤先 `PREPARE`，chain 全成功才 `COMMIT`（强一致但性能差）
- **D. 混合策略**：可逆步骤自动回滚；不可逆步骤强制 `PREPARE + COMMIT` 或触发补偿 Saga

**决策维度**：一致性（C>D>B>A）、自动化（B/D>A）、性能（A>B>D>C）、运维（A<B<D<C）

**关联 IC**：IC-L2-06（Supervisor 通知）、IC-09（补偿审计）、L1-04 WP 接口

**期望决策时间**：M4 前（影响 L1-04 WP 执行器的事务设计）

---

### OQ-04 · DAG 动态并行度

**问题**：DAG 执行当前按拓扑顺序串行。并行度（同一拓扑层的 steps 可否并行执行）如何决定？

**方案选项**：
- **A. 全串行**：最简单，debug 友好，性能差
- **B. 同层全并行**：Kahn 算法同层 steps 全并行（最大吞吐，但资源峰值高）
- **C. 按资源预算动态**：运行时根据 CPU/内存余量动态决定并行数
- **D. DAG-declared**：step 级声明 `parallel_group`，相同 group 并行（显式控制）

**决策维度**：吞吐（C>B>D>A）、资源峰值（A<D<C<B）、可预测性（A>D>B>C）

**关联 IC**：IC-L2-01（tick 调度）、L1-04 WP 执行器配额

**期望决策时间**：M3 后（M2 先串行跑通）

---

### OQ-05 · Chain 取消的扩散策略

**问题**：Chain 被 `abort_chain` 时，正在执行的步骤（可能在远端 executor）如何取消？

**方案选项**：
- **A. Best-effort 信号**：发 SIGTERM，超时后放弃（可能泄漏进程）
- **B. 强制 kill**：发 SIGKILL（泄漏进程状态，下次启动可能冲突）
- **C. 协作式取消**：步骤定期检查 `chain.cancelled` flag，主动退出（依赖步骤配合）
- **D. 两阶段取消**：先 signal + grace period 2× timeout，再 force kill

**决策维度**：及时性（B>D>A>C）、数据一致性（C>D>A>B）、实现复杂度（A<B<D<C）

**关联 IC**：L1-04 WP 执行器、IC-L2-03（状态同步）

**期望决策时间**：M3 前（影响取消端到端测试）

---

### OQ-06 · Partial Rollback 的顺序语义

**问题**：当 `rollback_chain` 触发时，已执行的步骤按 LIFO（后进先出）还是按反向拓扑排序回滚？

**方案选项**：
- **A. 严格 LIFO**：按 `executed_steps` 列表倒序（简单但可能违反依赖）
- **B. 反向拓扑**：按 DAG 反向拓扑序（尊重依赖，正确但可能阻塞）
- **C. 并行反向拓扑**：同层 steps 并行回滚（快但资源峰值高）
- **D. 指定回滚点**：`rollback_to_step(step_id)` 回到指定点（灵活但语义复杂）

**决策维度**：语义正确性（B/C/D>A）、速度（C>A>B>D）、可控性（D>A>B>C）

**关联 IC**：IC-L2-03（状态机 S→S2 回退）、IC-09（回滚审计）

**期望决策时间**：M4 前（影响 L1-04 WP 回滚设计）
