...
| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `L2_05_MOCK=true` | false | 激活审计记录器 Mock 模式 |
| `L2_05_MOCK_NOOP_FLUSH` | true | Mock 模式下 flush 为无操作（不写磁盘）|
| `L2_05_MOCK_FIXTURE_PATH` | — | 从 fixture 文件加载已有审计记录 |
...
class InMemoryAuditRecorder:
    """
    审计记录器测试替身。所有记录存内存，不写磁盘，支持 replay 验证。
    """
    def __init__(self, fixture_path: str | None = None):
        self._records: list[dict] = []
        self._flushed: list[dict] = []
        if fixture_path:
            self._records = self._load_fixture(fixture_path)

    def record_audit(self, entry: dict) -> str:
        entry["_hash"] = hashlib.sha256(json.dumps(entry, sort_keys=True).encode()).hexdigest()[:8]
    ...
    def query_by_tick(self, tick_id: str) -> list[dict]:
        return [r for r in self._flushed if r.get("tick_id") == tick_id]
```
...
| IC 方法 | Stub 行为 |
|---|---|
| `IC-09.append_event` | 追加到内存列表，不写 IC-09 Event Bus |
| `IC-L2-05.record_audit` | 写入 `_records` 列表 |
| `IC-L2-05.flush_buffer` | 移至 `_flushed`，不写 `.jsonl` 文件 |
| `IC-L2-05.replay_from_jsonl` | 从 fixture YAML 加载，不读文件系统 |

### §9.4 Fixture 示例

```yaml
# tests/fixtures/l2_05_audit_records.yaml
...
| # | 风险点 | 威胁类型 | 缓解措施 |
|---|---|---|---|
| SA-01 | 审计记录被事后删除或修改 | 审计篡改 | Append-only JSONL + SHA-256 hash-chain，每条记录含前条哈希；replay 验证完整性 |
| SA-02 | 跨 project_id 审计记录查询 | 权限越界 / 信息泄露 | `query_by_tick / query_by_decision` 强制校验 `project_id` 归属；跨项目查询 → `E_AUDIT_ACCESS_DENIED` |
| SA-03 | 审计缓冲区溢出导致记录丢失 | 数据丢失 / 审计空洞 | 缓冲区满（64条）时同步 flush，flush 前不接受新记录；`E_AUDIT_BUFFER_FULL` 触发告警 |
| SA-04 | 敏感决策内容明文存储 | 隐私泄露 | 敏感字段（user token、API key）在写入前脱敏（`***`），audit payload 禁含凭据 |
| SA-05 | replay 攻击（重复写入相同 tick_id） | 重放污染 | `record_audit` 检查 `tick_id` + `decision_type` 唯一性；重复写入 → `E_AUDIT_DUPLICATE_ENTRY` |

**安全审计频率：** SA-01（hash-chain 完整性）需每日自动 replay 校验（CI cron）。SA-04（敏感字段脱敏）需每次新增 `event_type` 时人工审查 whitelist。
...
| # | 问题 | 影响范围 | 期望决策时间 |
|---|---|---|---|
| OQ-01 | 审计记录是否需要静态加密（AES-256 或类似）？当前设计仅脱敏敏感字段，但 JSONL 文件本身明文存储。若需加密，密钥管理方案需在 M4 前确定。 | 数据安全 / 合规 | M4 前 |
| OQ-02 | Hash-chain 校验算法当前为 SHA-256。是否考虑迁移到 BLAKE3（性能提升 3-5x）？迁移涉及历史记录兼容性问题。 | 性能 / 安全 | R5 前 |
| OQ-03 | `query_by_decision` 的全量扫描在 project 审计记录超过 10 万条时 P95 是否仍满足 ≤5ms SLO？需要补充索引策略（B-tree on decision_type + timestamp）或分页 API 设计。 | 查询性能 | M4 验证 |
