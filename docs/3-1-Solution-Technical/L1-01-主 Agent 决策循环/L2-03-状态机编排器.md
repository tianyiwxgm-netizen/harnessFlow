...
| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `L2_03_MOCK=true` | false | 激活状态机编排器 Mock 模式 |
| `L2_03_MOCK_TRANSITION_DELAY_MS` | 0 | Mock 转换延迟（测试竞态用）|
| `L2_03_MOCK_DETERMINISTIC` | true | 确定性执行，忽略外部 IC 结果 |
...
class MockStateMachineOrchestrator:
    """
    状态机编排器测试替身。预加载转换序列，不依赖真实 L2-02/L2-04 响应。
    """
    def __init__(self, preset_transitions: list[tuple[str, str]]):
        """
        preset_transitions: [(from_state, to_state), ...]
        按顺序消费，模拟确定性状态转换序列。
        """
        self._transitions = list(preset_transitions)
    ...
    def apply_transition(self, trigger: str, project_id: str) -> dict:
        ...
        record = {"from": from_state, "to": to_state, "trigger": trigger, "project_id": project_id}
...
| IC 方法 | Stub 行为 |
|---|---|
| `IC-L2-03.apply_transition` | 从预设序列消费，立即返回 |
| `IC-L2-03.query_state` | 返回 `_current_state` |
| `IC-L2-03.get_valid_transitions` | 返回固定允许集 `[S1→S2, S2→S3, ...]` |
| IC-01 state change event | 静默（不发布到 Event Bus）|

### §9.4 Fixture 示例

```yaml
# tests/fixtures/l2_03_state_transitions.yaml
...
| # | 风险点 | 威胁类型 | 缓解措施 |
|---|---|---|---|
| SA-01 | 非法状态转换触发（如 S7 → S1 回滚到初始） | 状态机旁路 | 每次 `apply_transition` 前查询 `get_valid_transitions`，无效转换 → `E_TRANS_INVALID` |
| SA-02 | 并发转换竞争（同一 project_id 两个转换同时触发） | 竞态条件 / 状态撕裂 | project_id 粒度分布式锁，Lock TTL = 5s，超时 → `E_TRANS_LOCK_CONTENTION` |
| SA-03 | 外部调用跳过 guard 条件直接写状态 | 权限绕过 | guard 函数不可被外部直接覆盖；状态写入仅通过 `apply_transition` 入口 |
| SA-04 | HALTED 状态下强制触发 RESUME | 监督失效 | HALTED → 非 PAUSED/非继续状态，RESUME 需要 Supervisor IC-14/IC-15 双重确认 |
| SA-05 | 状态历史记录被篡改（EventSourcing 回放攻击） | 审计不可信 | 状态历史写 append-only JSONL，hash-chain 保护，篡改 → replay 验证失败 |

**安全审计频率：** SA-02（锁竞争）需每月监控 Prometheus 指标 `l2_03_lock_contention_total`。
...
| # | 问题 | 影响范围 | 期望决策时间 |
|---|---|---|---|
| OQ-01 | 状态转换是否需要强幂等性（同一触发器连续两次 → 第二次 no-op 还是报错）？当前设计报 `E_TRANS_INVALID`，但某些重试场景期望 no-op。 | 重试语义 | M3 前 |
| OQ-02 | `HALTED` 状态是否允许读取（query_state），还是 HALTED 后连查询也被阻塞？对 Supervisor 的可见性有影响。 | 监控可观测性 | M2 验证 |
| OQ-03 | 状态快照（snapshot）存储多少历史版本？无限存储会导致存储膨胀；若按版本数裁剪，最小应保留多少以支持 replay 审计？ | 存储策略 | R4 前 |
