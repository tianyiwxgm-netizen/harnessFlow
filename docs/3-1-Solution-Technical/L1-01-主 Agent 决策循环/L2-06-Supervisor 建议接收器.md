...
**实现**：本 L2 写 active_blocks[] → 调 L2-01 `on_hard_halt(cancel_signal)` → L2-01 发 async_cancel 给 L2-02 → 返回测量 latency_ms
...
| `E_SUP_L2_01_UNREACHABLE` | L2-01 `on_hard_halt` 抛异常或超时 | 降级：本 L2 强制 halt 标志 + 直接发 `L1-01:hard_halt` 事件 + 告警 `supervisor_gateway_degraded` 让 UI 感知 |
...
```python
    def broadcast_block(self, block_signal: BlockSignal) -> dict:
        ...
        try:
            l201_result = self.l201.on_hard_halt(
                cancel_signal,
                # 语义 · L2-01 收到后：
...
class TickScheduler:  # L2-01
    ...
    def on_hard_halt(self, cancel_signal: dict, deadline_ms: int) -> dict:
...
- **双 Event 通道**（threading.Event + asyncio.Event）：专属 BLOCK 信号——L2-01 承载；本 L2 在 broadcast_block 触发时调 L2-01.on_hard_halt，由 L2-01 同时 set 两个 Event。
...
| 当前 | 触发 | Guard | Action | 下一 |
|---|---|---|---|---|
| BS_NoBlock | broadcast_block(new) | 非幂等（red_line_id 新）| active_blocks.append · counter.block++ · l201.on_hard_halt · audit hard_halt_received | BS_BlockPending |
...
2. **L2-06 ↔ L2-02 的"BLOCK 抢占"**（最关键 · arch §3.5 D-05 硬约束）：
   - L2-06 `broadcast_block` → 调 L2-01 `on_hard_halt`
...
| L2-01 on_hard_halt（主动调 · IC-L2-08 语义）| L1-01 architecture.md §6 IC-L2-08 |
