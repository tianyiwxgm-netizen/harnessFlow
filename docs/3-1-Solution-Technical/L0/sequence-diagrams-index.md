...
> **定位**：HarnessFlow 所有 P0（核心）+ P1（重要）+ P2（异常）端到端链路时序图的**统一索引**。每条链路在本文件有"**一句话场景 + 主参与者 + Mermaid 骨架**"，**完整深度图**在对应 L1-XX / L2-YY 的 tech-design.md 或 integration tech-design 中。
> **消费方**：57 个 L2 tech-design 写"§5 P0/P1 核心时序图"时**直接引用本索引的骨架**；4 份 integration 文档**深化**本索引的 P0/P1 图；3-2 TDD 集成测试用例从本索引**派生测试场景**。
> **与 scope §8.3 / L1 集成 §5 的关系**：scope §8.3 给出 5 场景的**业务纵览**（文字形式），L1 集成 §5 给出 12 场景的**集成步骤**（编号形式），本文件给出**统一 Mermaid 骨架 + 深度图指针**（技术形式），三者互补。
...
|---|---|---|---|---|
| **P0 · 核心** | 贯穿项目主干 / 每个 V1 项目必跑 / 任何一条断掉产品不可用 | 集成测试必测 + 验收必演 + 架构必画 | **8 条** | 15~25 行 Mermaid 骨架 + 10+ 条 step |
| **P1 · 重要** | 高频触发 / 重要变体 / 典型异常补偿 / V2+ 扩展入口 | 集成测试必测 + 架构必画 | **12 条** | 8~15 行 Mermaid 骨架 |
| **P2 · 异常** | 低频 / 降级 / 红线 / 容灾（BF-E-01~12 兜底）| 诊断参考 + 韧性验证 | **6 条** | 5~10 行 Mermaid 骨架 |
...
sequenceDiagram
    autonumber
    participant U as 用户
    participant UI as L1-10 UI
    participant L01 as L1-01 主loop
    participant L02 as L1-02 生命周期
    participant L05 as L1-05 Skill调度
    participant L09 as L1-09 事件总线
    ...
    Note over L02: 生成 harnessFlowProjectId<br/>(slug + uuid-short)
...
sequenceDiagram
    ...
    participant L01 as L1-01 主loop
    participant L02 as L1-02 生命周期
    participant L03 as L1-03 WBS调度
    participant L04 as L1-04 Quality Loop
    participant L07 as L1-07 监督
...
sequenceDiagram
    autonumber
    participant OS as Claude Code 启动
    participant L09 as L1-09 韧性+审计
    participant L02 as L1-02 生命周期
    participant L01 as L1-01 主loop
    participant UI as L1-10 UI
    participant U as 用户
...
sequenceDiagram
    ...
    Note over L01: PAUSED 期间：<br/>不派发新 tick<br/>但事件总线仍收异步结果（subagent 回收等）
    U->>UI: 点 resume
    UI->>L01: IC-17 user_intervene(type=resume)
...
sequenceDiagram
    ...
    Note over L04,L07: 若双方判定一致 → 执行<br/>若不一致 → 取更严级别
    L04->>L01: 请求 state 转换
    ...
    Note over L04: 同级 FAIL 计数 += 1<br/>若 ≥ 3 → 自动升级一级（详见 P1-04）
```
...
sequenceDiagram
    ...
    Note over L04: 若升级后仍 3 次 FAIL → 再升级（L2→L3→L4）<br/>L4 3 次 → state=FAILED_TERMINAL → S7 失败闭环
```
...
sequenceDiagram
    autonumber
    participant Caller as 调用方（L1-01/04）
    participant L05 as L1-05 Skill调度
    participant L09 as L1-09 事件总线
    participant L01 as L1-01 主loop
...
sequenceDiagram
    autonumber
    participant Caller as 调用方（L1-01/04/08）
    participant L05 as L1-05 Skill调度
    participant SA as 子 Agent（独立 session）
    participant L09 as L1-09 事件总线
    participant L01 as L1-01 主loop
...
sequenceDiagram
    autonumber
    participant AnyL as 任意 L1（01-09）
    participant L09 as L1-09 事件总线
    participant L10 as L1-10 UI
...
sequenceDiagram
    ...
    Note over L06: S1→trap + pattern<br/>S2→recipe + tool_combo<br/>S3→anti_pattern<br/>S4→pattern<br/>S5→trap<br/>S7→反向收集
...
sequenceDiagram
    ...
    L07->>L07: 检测：<br/>state 偏差 > 30% ?<br/>goal_anchor 命中率 < 60% ?<br/>"暂时跳过" ≥ 5 次 ?
    alt 触发 soft-drift
        L07->>L01: IC-13 push_suggestion(WARN, dimension=goal_fidelity, suggested_action=replan)
        L01->>L01: 书面回应：采纳 replan 或驳回+理由
    end
    Note over L07: soft-drift 区别于硬红线 DRIFT_CRITICAL<br/>(后者 sha256 变化硬拦截，走 P0-06 变体)
```
...
sequenceDiagram
    ...
    L09->>L09: 尝试落盘
    L09--xL09: 写盘失败（磁盘满 · 权限错 · fsync 失败）
    Note over L09: PM-08 单一事实源不可破<br/>halt 整个系统
    L09->>L01: halt 全系统
    L01->>L01: state = SYSTEM_HALTED
    L09->>UI: 强告警 + 诊断信息
    Note over L09: 恢复路径：<br/>用户修磁盘 / 权限后<br/>重启 Claude Code → 走 P0-05 bootstrap
```
...
每条时序图必须可 **Mermaid Live Editor**（mermaid.live）**直接渲染通过**，不得有语法错误（CI 可加 mermaid-cli lint）。
...
- **官方文档 `docs/concepts/agentic_concepts.md`** 用 Mermaid 时序图展示 `human_in_the_loop` / `multi_agent` / `subgraph` 的调用流
- **参考点**：
...
sequenceDiagram
    autonumber
    participant Caller
    participant L05 as L1-05
    participant SA as 子 Agent
    participant L09 as L1-09
...
sequenceDiagram
    autonumber
    participant L07 as L1-07
    participant L01 as L1-01
    L07->>L07: 判定等级
    alt INFO
