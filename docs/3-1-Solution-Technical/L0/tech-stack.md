...
### 1.4 零外部依赖红线（修订 2026-04-23 · Vite dev 允许 · Prod 仍零 Node）

**HarnessFlow 作为开源 Claude Code Skill · End User 部署端硬性约束**：

- ✅ **End User 运行态（Prod）零 Node**：UI 分发为预编译静态 HTML/JS/CSS · 随 repo 一起发 · 用户**无需**装 Node/npm
- 🟡 **开发态（Dev · 贡献者 & 维护者）允许 Vite/npm**：Vue 3 源文件需 build（`npm run build` → `frontend/dist/` 静态资产）· CI 出 release 时包含 dist · End User 不触发 build
...
- ✅ End User 只需：Claude Code 平台原生 + Python stdlib + 可控 pip 包（fastapi / uvicorn / jsonschema / httpx）+ 预编译前端静态资产
- ✅ Dev 另需：Node 20+ · npm · Vite（仅 build dist/ 时）

**理由**：
- **Goal §2.1 产品形态仍是开源 Claude Code Skill** · End User 部署 0 Node 摩擦维持不变
- Vite 作为现代 Vue 3 官方工具链 · Dev 侧效率显著高于 CDN（热更新 / TS 原生 / 组件库按需 / 生产 bundle 体积小 40%+）
- **分发策略**：`frontend/dist/` 进 git（或 CI release 产出）· End User pip install 后直接由 FastAPI `StaticFiles` 挂载 · 访问 `http://localhost:8000/ui/` 即得 UI · 无 `npm install` / `npm run build` 负担

**AD-05 历史注释**（原"零 npm 全 CDN"）已被此条 2026-04-23 修订取代 · 原因：Vue 3 生态已深度依赖打包工具 · CDN 模式难以维持类型安全 + 单文件组件（SFC）· 实测同等 UI 功能 CDN 模式需 3-5 倍的纯手写 JS · 长期维护成本高。

**此修订经主会话仲裁 · 原 L0/tech-stack §131 硬红线放宽 · 参见**：
- `docs/4-exe-plan/arbitration-logs/2026-04-23-Dev-θ-4-corrections.md`（Dev-θ C-1 仲裁）
- `projects/_correction_log.jsonl`（`θ-C1` 记录）
