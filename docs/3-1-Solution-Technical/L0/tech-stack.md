...
```mermaid
graph TB
    subgraph L1_Runtime["第 1 层 · Skill 核心运行时"]
        CC[Claude Code CLI]
        SK[主 skill · markdown]
        SUB[subagents · markdown]
        CMD[slash commands · markdown]
        CC --> SK
        CC --> SUB
        CC --> CMD
    end

    subgraph L2_Tooling["第 2 层 · 工具 + 编排"]
        BASH[bash hooks]
        PY[Python primitives]
    ...
    subgraph L3_Storage["第 3 层 · 持久化"]
        JSONL[事件 jsonl]
        JSON[task-board JSON]
        YAML[matrix / state YAML]
        MD[产出物 md]
        SCHEMA[JSON Schema]
        PY --> JSONL
        BASH --> JSONL
    ...
    subgraph L5_Ecosystem["第 5 层 · 生态（引用）"]
        SP[superpowers]
    ...
    style L1_Runtime fill:#e1f5fe
    style L2_Tooling fill:#fff9c4
    style L3_Storage fill:#f3e5f5
    style L4_UI fill:#e8f5e9
    style L5_Ecosystem fill:#fce4ec
```
...
### 2.6 主 skill · subagent · hook 三者通信时序（Mermaid）
...
sequenceDiagram
    ...
    participant Main as 主 skill (harnessFlow.md)
    participant Sup as Supervisor subagent
    participant Ver as Verifier subagent
    participant Hook as PostToolUse hook (bash)
    participant Disk as 文件系统 (task-boards/*.jsonl + md)
...
```mermaid
graph LR
    subgraph MainSession["主 session (Claude Opus)"]
        MainPrompt[主 skill prompt]
        MainCtx[context 副本 A]
        MainState[task-board 读写权]
    end

    subgraph SupSession["Supervisor session (独立)"]
        SupPrompt[supervisor.md prompt]
        SupCtx[context 副本 B · 只读]
        SupState[只能读 task-board]
    end

    subgraph VerSession["Verifier session (独立)"]
        VerPrompt[verifier.md prompt]
        VerCtx[context 副本 C · 只读 + 限工具]
        VerState[只能读 artifacts]
    end

    MainPrompt -.Task 参数注入.-> SupPrompt
    MainPrompt -.Task 参数注入.-> VerPrompt
    MainState --> SupState
    MainState --> VerState

    style MainSession fill:#bbdefb
    style SupSession fill:#fff9c4
    style VerSession fill:#c8e6c9
```
...
### 4.8 UI 的关键数据流（Mermaid）

```mermaid
sequenceDiagram
    participant User as 用户浏览器
    participant Vue as Vue 3 前端
    participant FAPI as FastAPI backend
    participant FS as 文件系统
    participant Main as 主 skill session
...
### 6.6 hook 执行模型（Mermaid）

```mermaid
sequenceDiagram
    participant Claude as Claude 模型
    participant CC as Claude Code runtime
    participant Hook as bash hook 脚本
    participant Py as python3 -c 一行
    participant FS as supervisor-events.jsonl
...
### 7.8 Skill 生态的依赖关系（Mermaid）

```mermaid
graph TB
    subgraph HarnessFlow["HarnessFlow 自建"]
        Main[主 skill]
        Sup[Supervisor]
        Ver[Verifier]
        Retro[Retro-generator]
        FAW[Failure-archive-writer]
    end

    subgraph SP["superpowers (SP)"]
        BR[brainstorming]
        WP[writing-plans]
...
锚 3-solution-design spec §6.4：每份 tech-design 至少 1 张 Mermaid 时序图。
...
graph TD
    ...
    VUE[Vue 3.4+ CDN]
    EP[Element Plus 2.5+ CDN]
    MER[Mermaid 10+ CDN]
    MARK[marked 11+ CDN]

    BROWSER[Chrome 110+ / Firefox 115+ / Safari 16+]

    HF[HarnessFlow Skill]
    ...
    style HF fill:#e1f5fe,stroke:#01579b,stroke-width:3px
    style CC fill:#fff9c4
    style PY fill:#c8e6c9
    style BASH fill:#ffccbc
