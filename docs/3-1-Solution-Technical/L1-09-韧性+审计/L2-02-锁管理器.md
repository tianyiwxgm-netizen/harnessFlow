...
### 13.1 `docs/2-prd/L1-09 韧性+审计/prd.md` §9.9 GWT 场景 → 本文档章节 → 3-2 用例

> **3-2 TDD 规格文件**：`docs/3-2-Solution-TDD/L1-09-韧性+审计/L2-02-锁管理器-tests.md`（待建 · 本文档所有 GWT 场景 + §11 错误码 + §12 SLO 的用例清单）
>
> 下表的 `tests/l1_09/l2_02/*.py` 是 TDD 实施层 pytest 文件路径 · 在 3-2 TDD 规格文件中按 TC ID 组织（TC ID 见 §13.5）。
...
### 13.2 `docs/2-prd/L1-09 韧性+审计/prd.md` §9.6 ✅ 必须清单 → 本文章节
...
### 13.3 `docs/2-prd/L1-09 韧性+审计/prd.md` §9.5 🚫 禁止清单 → 本文章节
...
### 13.5 3-2 Detail 层任务清单（带 TC ID · 建议落地顺序）

命名风格：**`T-LOCK-<CATEGORY>-NNN`**（与 L2-04 `T-RECOVER-*` 风格严格对齐 · 详见 §13.5.1 命名说明）。所有 TC ID 在 `docs/3-2-Solution-TDD/L1-09-韧性+审计/L2-02-锁管理器-tests.md` 反向落实。

| TC ID | 3-2 任务 / pytest 文件 | 说明 / 关联章节 |
|---|---|---|
| —（fixture） | `tests/l1_09/l2_02/conftest.py` | 构造 LockManager fixture · tmp workdir · 注入配置 |
| `T-LOCK-HAPPY-001` | `tests/l1_09/l2_02/test_acquire_no_contention.py` | 场景 1 · 无竞争 acquire/release |
| `T-LOCK-HAPPY-002` | `tests/l1_09/l2_02/test_release_idempotent.py` | §3.3 `ok_idempotent` / `ok_forced_released` |
| `T-LOCK-VALIDATION-001` | `tests/l1_09/l2_02/test_resource_validation.py` | §3.2 前置条件 · 不合法 resource/holder/token |
| `T-LOCK-FIFO-001` | `tests/l1_09/l2_02/test_fifo_10_contenders.py` | 场景 2 · 10 方 FIFO 公平性 |
| `T-LOCK-TIMEOUT-001` | `tests/l1_09/l2_02/test_timeout.py` | 场景 3 · 等锁超时 + 退避 |
| `T-LOCK-DEADLOCK-001` | `tests/l1_09/l2_02/test_deadlock_detect.py` | 场景 4（2-cycle + 3-cycle）|
| `T-LOCK-SHUTDOWN-001` | `tests/l1_09/l2_02/test_shutdown.py` | 场景 5 · 干净 shutdown 释放所有锁 |
| `T-LOCK-JANITOR-001` | `tests/l1_09/l2_02/test_janitor_ttl.py` | §6.4 + §11.2.6 锁泄漏 + janitor TTL |
| `T-LOCK-FSERR-001` | `tests/l1_09/l2_02/test_fs_error.py` | §11.2.5 盘满 / EROFS 模拟 |
| `T-LOCK-BENCH-001` | `tests/l1_09/l2_02/bench_acquire_release.py` | §12.2 Bench 1 · acquire/release P95 |
| `T-LOCK-BENCH-002` | `tests/l1_09/l2_02/bench_10_contenders.py` | §12.2 Bench 2 · 10 方竞争 P95 |
| `T-LOCK-BENCH-003` | `tests/l1_09/l2_02/bench_20_starvation.py` | §12.2 Bench 3 · 20 方 10s 无饥饿 |

**合计 12 个 TC ID + 1 个 fixture · 5 类覆盖**：HAPPY (2) / VALIDATION (1) / FIFO (1) / TIMEOUT (1) / DEADLOCK (1) / SHUTDOWN (1) / JANITOR (1) / FSERR (1) / BENCH (3)。

#### 13.5.1 TC ID 命名规范

- 格式：`T-LOCK-<CATEGORY>-<NNN>`（与 L2-04 `T-RECOVER-*` / L2-03 `T-AUDIT-*` 同族）
- DOMAIN 段：`LOCK`（本 L2 唯一）
- CATEGORY 段：HAPPY / VALIDATION / FIFO / TIMEOUT / DEADLOCK / SHUTDOWN / JANITOR / FSERR / BENCH
- 序号：3 位零填充（001-999）· 同 CATEGORY 内单调
- 反向定位：`grep -rn "T-LOCK-FIFO-001" docs/3-2-Solution-TDD/L1-09-韧性+审计/`
...
- [x] §1 定位 + 2-prd 映射 · 覆盖 `docs/2-prd/L1-09 韧性+审计/prd.md` §9.1 / §9.8 / §9.9
