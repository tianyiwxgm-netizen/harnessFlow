...
### 13.1 `docs/2-prd/L1-09 韧性+审计/prd.md` §12.9 交付验证大纲 → 3-2 TDD 测试文件

> **3-2 TDD 规格文件**：`docs/3-2-Solution-TDD/L1-09-韧性+审计/L2-05-崩溃安全层-tests.md`（待建 · 本文档 7 场景 + §11 错误码 + §12 SLO 的用例清单）
>
> 下表 `tests/l1_09/l2_05/*.py` 是 TDD 实施层 pytest 文件路径 · 在 3-2 TDD 规格文件中按 TC ID 组织（TC ID 见 §13.X 表新增列）。

`docs/2-prd/L1-09 韧性+审计/prd.md` §12.9（第 1528-1578 行）定义 7 个 Given-When-Then 场景 · 本 L2 承诺每个场景至少 1 个 TDD 测试文件（由 3-2 层落地）：

| # | TC ID | prd 场景 | 测试用例文件（3-2 路径）| 断言要点 | 对应本文档章节 |
|---|---|---|---|---|---|
| 1 | `T-CRASH-HAPPY-001` | 原子追加正常 | `tests/l1_09/l2_05/test_append_happy_path.py` | 5 步完成 · fsync 调用 · 返 AppendResult · P95 ≤ 20ms | §5.1 + §6.2 |
| 2 | `T-CRASH-CORRUPT-001` | 中断不坏数据（断电测试） | `tests/l1_09/l2_05/test_crash_safety_simulation.py` | 用 `os.kill(pid, SIGKILL)` + 重启 · 校验 N-1 条完整 · 无半行 | §6.1 + §8.2 不变量 |
| 3 | `T-CRASH-DISK-001` | 磁盘满触发硬 halt | `tests/l1_09/l2_05/test_disk_full_hard_halt.py` | mock `os.write` 抛 ENOSPC · retry 2 次 · 抛 `CrashSafetyDiskFullError` | §5.3 + §11.5 |
| 4 | `T-CRASH-HASH-001` | hash 链断裂检测 | `tests/l1_09/l2_05/test_hash_chain_corruption.py` | 手工破坏第 N 条 hash · `verify_integrity` 返 CORRUPT/PARTIAL | §6.3.3 + §5.2 |
| 5 | `T-CRASH-HEADER-001` | checkpoint header 损坏 | `tests/l1_09/l2_05/test_header_checksum_failure.py` | 破坏 header 64 bytes · 返 CORRUPT | §6.4 `_verify_header_checksum` |
| 6 | `T-CRASH-SHUTDOWN-001` | shutdown 末自检 | `tests/l1_09/l2_05/test_shutdown_selfcheck.py` | 干净 shutdown 后 `verify_integrity` 返 OK | §5.2 |
| 7 | `T-CRASH-PERF-001` | 一万事件校验延迟 | `tests/l1_09/l2_05/test_10k_events_verify_sla.py` | 生成 10000 事件 jsonl · `verify_integrity` ≤ 5000ms | §6.3.4 + §12.1 |

**合计 7 个 TC ID · 7 类覆盖**（HAPPY / CORRUPT / DISK / HASH / HEADER / SHUTDOWN / PERF），命名风格 `T-CRASH-<CATEGORY>-NNN`（与 L2-04 `T-RECOVER-*` / L2-03 `T-AUDIT-*` / L2-02 `T-LOCK-*` 同族）。所有 TC ID 在 `docs/3-2-Solution-TDD/L1-09-韧性+审计/L2-05-崩溃安全层-tests.md` 反向落实。
...
| `docs/2-prd/L1-09 韧性+审计/prd.md` §12.4 本地重试 2 次 | §6.1 + §10.1 `RETRY_MAX=2` | 硬 cap · 超出抛错 |
| `docs/2-prd/L1-09 韧性+审计/prd.md` §12.5 禁延迟 fsync | §10.1 `FSYNC_MODE=always` | 配置守护 |
| `docs/2-prd/L1-09 韧性+审计/prd.md` §12.6 三态完整性 | §3.4 + §7.4 | `IntegrityState` enum · 必 3 选 1 |
