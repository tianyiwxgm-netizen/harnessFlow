...
| 本 L2 方法 / § 段 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9 锚点 | 映射类型 |
|---|---|---|
| `load_topology`（IC-L2-01）| `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.2 输入"WBS 拓扑数据装图请求" + §9.6 必须 "DAG 无环校验" | 处理器 |
| `read_snapshot`（IC-L2-02）| `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.2 输入"节点状态快照读请求" + §9.6 必须 "暴露拓扑只读数据" | 处理器 |
| `transition_state`（IC-L2-03）| `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.2 输入"状态锁定请求" + §9.4 硬约束 2/3/5 | 处理器 |
| `transition_state`（IC-L2-04）| `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.2 输入"状态跃迁申请" + §9.6 必须 "封闭六状态跃迁合法性校验" | 处理器 |
| `mark_stuck`（IC-L2-06）| `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.2 输入"stuck 标记申请" | 处理器 |
| `on_system_resumed` | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.6 必须 "响应 system_resumed 触发重建" | 处理器 |
| `export_readonly_view` | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.6 必须 "向 L1-10 暴露可视化只读数据" + "向 L1-07 暴露进度节奏基础数据" | 处理器 |
| §2 DDD 聚合根 7 不变量 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.4 硬约束 1-7 | DDD 落地 |
| §8 六状态机 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.4 硬约束 3 | 状态机落地 |
| §11 错误处理降级链 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.5 禁止 1-8 + `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.10 异常处理文字 | 错误语义落地 |

### 13.2 § 段 ↔ `docs/3-2-Solution-TDD/L1-03-WBS+WP 拓扑调度/L2-02-tests.md`（前向占位 · 待建 · TC ID 矩阵 ≥ 15）

| # | TC ID | 覆盖 § | G-W-T 对应 | 用例定性 |
|---|---|---|---|---|
| 1 | TC-L103-L202-P01 | §6.1 + §5.1 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.9 P1 | 正常装图 · 合法 DAG → ok + critical_path |
| 2 | TC-L103-L202-P02 | §6.2 + §5.2 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.9 P2 | READY→RUNNING 锁定 · deps_met + count<2 → ok |
| 3 | TC-L103-L202-P03 | §6.2 + §11.1 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.9 P3 | 第 3 个锁定 → E_301 并行超上限 |
| 4 | TC-L103-L202-P04 | §6.2 + §8.3 T3 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.9 P4 | RUNNING→DONE · count 减 1 + lock 释放 |
| 5 | TC-L103-L202-P05 | §6.3 + §5 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.9 P5 | 跨 session 重建 · checkpoint + events 回放 |
| 6 | TC-L103-L202-P06 | §6.4 + §6.1 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.9 P6 | 差量合并 · 保留 DONE + 新子图拼接 |
| 7 | TC-L103-L202-N01 | §6.1 + §11 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.9 N1 | DAG 有环 → E_101 + offending_nodes |
| 8 | TC-L103-L202-N02 | §11.1 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.9 N2 | 并发锁定 3rd → E_301 + parallel_running_wps |
| 9 | TC-L103-L202-N03 | §6.2 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.9 N3 | 依赖未 satisfied → E_302 awaiting_deps |
| 10 | TC-L103-L202-N04 | §6.2 + §8.2 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.9 N4 | READY→DONE 非法跃迁 → E_303 |
| 11 | TC-L103-L202-N05 | §11.3 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.9 N5 | bypass FS 直写 → E_501 + bypass_attempt 审计 |
| 12 | TC-L103-L202-N06 | §6.1 + §11 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.9 N6 | 运行时改依赖（非差量入口）→ rejected |
| 13 | TC-L103-L202-I01 | §4 + §5 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.9 I1 | 连续取 2 WP + 第 3 拒 + 第 1 DONE 后第 3 放开（跨 L2-03）|
| 14 | TC-L103-L202-I02 | §6.3 + §5 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.9 I2 | kill-restart · resume 至 kill 时 RUNNING 状态 |
| 15 | TC-L103-L202-I03 | §6.4 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §9.9 I3 | L2-05 → L2-01 差量 → L2-02 合并 · 关键路径刷新 |
