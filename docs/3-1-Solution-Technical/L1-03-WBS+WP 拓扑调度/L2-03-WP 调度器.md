...
| 本 L2 方法 / § 段 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10 锚点 | 映射类型 |
|---|---|---|
| `get_next_wp`（IC-02）| `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.2 输入 "IC-02 get_next_wp 调用" + §10.8 IC 交互 | 处理器 |
| Step 3 筛选（CandidateFilter）| `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.4 硬约束 1 "依赖 satisfied" + §10.6 必须 "按四条规则筛选" | 算法 |
| Step 2 并行度位（backpressure）| `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.4 硬约束 2 "并行 ≤ 2" | 算法 |
| Step 5 priority（critical_path first）| `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.4 硬约束 3 "关键路径优先" + §10.6 必须 | 算法 |
| Step 4 三态判定（TriStateDetector）| `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.4 硬约束 4 "三态不可合并" + §10.6 必须 | 算法 |
| Step 6 锁 + 再校验 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.4 硬约束 5 "选定即锁定" + §10.5 禁止 "返回前未锁定" | 算法 |
| 无状态 Application Service | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.4 硬约束 6 "无状态调度" | DDD 落地 |
| deadlock 通知 L2-05 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.6 必须 "deadlock 时通知 L2-05" + §10.8 | 处理器 |
| IC-L2-08 审计 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.6 必须 "审计调度决策" + §10.5 禁止 "不走审计" | 副作用 |
| `get_remaining_hint`（🔧）| `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.7 🔧 可选功能 1 "批量调度提示" | 辅助接口 |
| §11 错误处理降级链 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.5 禁止 1-8 + §10.10 异常处理 | 错误语义 |

### 13.2 § 段 ↔ `docs/3-2-Solution-TDD/L1-03-WBS+WP 拓扑调度/L2-03-tests.md`（前向占位 · 待建 · TC ID 矩阵 ≥ 17）

| # | TC ID | 覆盖 § | G-W-T 对应（`docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.9）| 用例定性 |
|---|---|---|---|---|
| 1 | TC-L103-L203-P01 | §5.1 + §6.1 Step 1-6 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.9 P1 | 所有 WP ready + 0 running · critical_path 首选 · 返回 wp_id |
| 2 | TC-L103-L203-P02 | §5.1 + §6.2 + §6.3 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.9 P2 | 1 running + 多 ready · 第 2 次取 · 并行度变 2 |
| 3 | TC-L103-L203-P03 | §6.2 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.9 P3 | 全部 DONE → `reason=all_done` · 无 L2-05 通知 |
| 4 | TC-L103-L203-P04 | §6.2 + §5.2b | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.9 P4 | 全 FAILED/BLOCKED/STUCK → `reason=deadlock` · 同步通知 L2-05 |
| 5 | TC-L103-L203-P05 | §6.2 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.9 P5 | 有 READY 但 deps 未 DONE → `reason=awaiting_deps` |
| 6 | TC-L103-L203-P06 | §6.3 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.9 P6 | 候选含 critical + non-critical · critical 优先被选 |
| 7 | TC-L103-L203-N01 | §6.1 + §11.3 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.9 N1 | WP-X deps=[WP-Y READY] · WP-X 不被选 · 返回 awaiting_deps |
| 8 | TC-L103-L203-N02 | §6.1 Step 2 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.9 N2 | running=2 · 不锁第 3 个 · 返回 awaiting_deps |
| 9 | TC-L103-L203-N03 | §5.2a + §6.4 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.9 N3 | L2-02 stale 拒绝 · 重新筛选 · 第 2 次成功 |
| 10 | TC-L103-L203-N04 | §6.7 + §11.1 E_501 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.9 N4 | 后台定时器 bypass 尝试 · 启动 guard 拒绝 · 审计 |
| 11 | TC-L103-L203-N05 | §3.3 + §11 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.9 N5 | 返回 `wp_id=null` 但 `reason=null` → 契约级禁止 · schema 校验失败 |
| 12 | TC-L103-L203-I01 | §5.1 + §5.2a 全链路 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.9 I1 | 取 WP → IC-03 → L1-04 执行 → wp_done → L2-04 打点 → 再 IC-02 取下一 |
| 13 | TC-L103-L203-I02 | §5.2b + L2-05 交互 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.9 I2 | 所有 WP FAILED · deadlock · L2-05 收通知 · failure_count≥3 触发 BF-E-08 |
| 14 | TC-L103-L203-I03 | §6.3 + §5.1 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §10.9 I3 | 拓扑双路径 · critical_path 持续优先到全 DONE · 非关键最后动 |
