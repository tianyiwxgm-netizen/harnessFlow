...
| 本 L2 方法 / § 段 | 2-prd §12 锚点 | 映射类型 |
|---|---|---|
| `on_wp_failed`（IC-L2-05）| prd §12.2 输入"L2-04 失败信号"+ §12.8 IC-L2-05 | 处理器 |
| `on_wp_done_reset`（IC-L2-05-B）| prd §12.4 硬约束 1 "wp_done 即清零" + §12.6 必须 2 | 处理器 |
| `on_deadlock_notified` | prd §12.2 输入"L2-03 deadlock" + §12.4 硬约束 5 "立即触发" | 处理器 |
| `on_rollback_route_chosen` | prd §12.2 输入"IC-14 回传" + §12.9 P3/P4 | 处理器 |
| `rebuild_counters_from_events` | prd §12.4 硬约束 7 + §12.6 必须 8 | 处理器 |
| `export_failure_summary` | prd §12 外补 · UI / Supervisor 观察需要 | 处理器 |
| §2 DDD FailureCounter Entity + RollbackAdvice VO | prd §12.6 必须 1 "按 wp_id 维护" + 4 "结构化建议" | DDD 落地 |
| §8 FailureCounter 5 状态机 | prd §12.4 硬约束 1 "连续语义" + §12.9 N1 断裂场景 | 状态机落地 |
| §11 错误处理降级链 | prd §12.5 禁止 1-8 + §12.6 必须 6 "evidence 非空" | 错误语义落地 |

### 13.2 § 段 ↔ `docs/3-2-Solution-TDD/L1-03-WBS+WP 拓扑调度/L2-05-失败回退协调器-tests.md` 前向映射（TC ID 矩阵 ≥ 15）

| # | TC ID | 覆盖 § | G-W-T 对应 | 用例定性 |
|---|---|---|---|---|
| 1 | TC-L103-L205-P01 | §6.1 + §5.1 | prd §12.9 P1 | 连续失败 2 次 · 第 3 次触发 BF-E-08 · 建议推 L1-01 · 标 stuck · 审计 |
| 2 | TC-L103-L205-P02 | §6.1 + §5.3 + §8.3 T4 | prd §12.9 P2 | 失败 2 次后 wp_done · counter 归零 · 新失败从 1 起算 |
| 3 | TC-L103-L205-P03 | §6.6 | prd §12.9 P3 | chosen_path=SPLIT_WP · 调 IC-L2-07 · L2-01 差量拆解 · 原 stuck wp 被替换 |
| 4 | TC-L103-L205-P04 | §6.6 | prd §12.9 P4 | chosen_path=MODIFY_AC · 保持 stuck · 等 L1-02 变更管理流 |
| 5 | TC-L103-L205-P05 | §6.5 + §11.3 第 9 场景 | prd §12.9 P5 | L2-03 死锁通知 · 立即 IC-15 hard_halt · 不延迟到下 tick |
| 6 | TC-L103-L205-P06 | §6.2 + §5 | prd §12.9 P6 | 跨 session 重建 · events.jsonl 按时序重放 · counter 还原 |
| 7 | TC-L103-L205-N01 | §6.2 + §5.3 + §8.3 T9 | prd §12.9 N1 | failed(2) → done → failed(1) · counter=1（非 3）· 不触发 BF-E-08 |
| 8 | TC-L103-L205-N02 | §10 + §11.1 | prd §12.9 N2 | env override failure_threshold=5 · 启动被拒（E_102）|
| 9 | TC-L103-L205-N03 | §3 guard + §11.1 | prd §12.9 N3 | 本 L2 擅自触发回退执行（调 L1-04 重跑）· 契约 API 不提供此方法 |
| 10 | TC-L103-L205-N04 | §6.4 + §11.1 | prd §12.9 N4 | 尝试直接改拓扑绕过 L2-02 · L2-02 一致性校验拒绝 |
| 11 | TC-L103-L205-N05 | §6.3 + §11.1 | prd §12.9 N5 | 建议无 evidence_refs · 契约级禁止 · 触发 E_301 IC-15 |
| 12 | TC-L103-L205-N06 | §3 guard + §11.1 | prd §12.9 N6 | 直接推卡给 UI · 契约 API 不提供此方法 |
| 13 | TC-L103-L205-I01 | §6.1 + §6.3 + §6.6 + §5.2 | prd §12.9 I1 | 整链：连续失败 3 → BF-E-08 → 建议"SPLIT_WP" → L1-07 → IC-L2-07 → L2-01 差量 → L2-02 合并 → L2-03 可取新子 WP |
| 14 | TC-L103-L205-I02 | §6.5 + §5 | prd §12.9 I2 | 整图死锁：所有 WP failed → L2-03 deadlock → L2-05 升级 → L1-07 分析 → L1-02 S2 重规划 |
| 15 | TC-L103-L205-I03 | §6.2 + §5 | prd §12.9 I3 | 跨 session：会话 A 某 WP 失败 2 次 → kill → 会话 B system_resumed → L2-05 重建 count=2 → 会话 B 再失败 1 → 触发 BF-E-08 |
...
|---|---|---|---|
| **OQ-L103-L205-01** | 是否支持"失败 2 次 WARN 预警"（未到阈值但值得关注）？| 当前未实现（prd §12.7 可选 2）· YAGNI 推迟 | M4 性能测试阶段 |
| **OQ-L103-L205-02** | 模式识别功能（多 WP 连续失败原因相似 → 共性建议）是否在 depth-C 补齐？| 当前仅 per-wp 独立建议（prd §12.7 可选 1）· YAGNI | R5 TDD 之后 |
| **OQ-L103-L205-03** | `failure-archive` jsonl 是否应该本 L2 主动写入（prd §12.7 可选 5）？| 当前归 L1-09 · 本 L2 仅产事件 · 待 L1-09 L2-05 failure-archive 方案确认后 | L1-09 depth-B 完成后对齐 |
