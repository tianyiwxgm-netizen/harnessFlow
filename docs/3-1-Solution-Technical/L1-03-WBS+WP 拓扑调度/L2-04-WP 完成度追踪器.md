...
| 本 L2 方法 / § 段 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11 锚点 | 映射类型 |
|---|---|---|
| `on_wp_done`（订阅）| `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.2 输入"wp_done" + §11.6 必须 "全量消费 wp_done" | 处理器 |
| `on_wp_failed`（订阅）| `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.2 输入"wp_failed" + §11.6 必须 "wp_failed 时同步 L2-05" | 处理器 |
| `on_system_resumed` | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.2 输入"system_resumed" + §11.6 必须 "跨 session 从事件总线重建" | 处理器 |
| IC-L2-04 发起 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.8 调方表 "IC-L2-04 申请状态跃迁" + §11.4 硬约束 4 "必经 L2-02" | 处理器 |
| IC-L2-05 发起 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.8 调方表 "IC-L2-05 同步失败信号" + §11.4 硬约束 3 | 处理器 |
| IC-L2-08（自产事件）| `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.8 调方表 "IC-L2-08 审计落盘" | 处理器 |
| `get_progress_metrics` | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.2 输出"暴露进度节奏基础指标" + §11.8 被调方表 "L1-07 数据拉取" | 处理器 |
| `export_readonly_view` | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.2 输出"L1-10 WBS tab 只读数据" + §11.8 被调方表 "L1-10 UI 只读数据拉取" | 处理器 |
| §2 DDD 无聚合根 + VO ProgressMetrics | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.4 硬约束 5 "可从事件总线重建" | DDD 落地 |
| §8 运维模式（非业务状态机）| `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.1 "只做记账 + 转发" | 无状态落地 |
| §11 错误处理降级链 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.5 禁止 1-8 + `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.10 异常处理文字 | 错误语义落地 |

### 13.2 § 段 ↔ `docs/3-2-Solution-TDD/L1-03-WBS+WP 拓扑调度/L2-04-WP 完成度追踪器-tests.md`（前向占位 · TC ID 矩阵 ≥ 17）

| # | TC ID | 覆盖 § | G-W-T 对应（`docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9）| 用例定性 |
|---|---|---|---|---|
| 1 | TC-L103-L204-P01 | §6.2 + §5.1 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9 P1 | wp_done 订阅 → IC-L2-04 ok → 并行度释放 → completion_rate 上升 → 推 UI + 推 L1-07 |
| 2 | TC-L103-L204-P02 | §6.3 + §5.2 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9 P2 | wp_failed 订阅 → 两路径并行 → L2-02 FAILED + L2-05 计数 → UI + L1-07 |
| 3 | TC-L103-L204-P03 | §6.2 + §6.4 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9 P3 | 10 个 WP 全 done → completion_rate = 1.0 · remaining_effort = 0 · done_wps 10 个 |
| 4 | TC-L103-L204-P04 | §6.1 + §8.2 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9 P4 | 跨 session 重启 → on_system_resumed → rebuild_from_events → metrics 重建至 kill 时 |
| 5 | TC-L103-L204-P05 | §3.6 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9 P5 | L1-10 调 export_readonly_view → 返回 metrics + 拓扑 merge hint |
| 6 | TC-L103-L204-P06 | §3.6 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9 P6 | L1-07 调 get_progress_metrics → 返回 metrics + parallelism_util |
| 7 | TC-L103-L204-N01 | §6.3 + §11.3 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9 N1 | wp_failed 未同步 L2-05（E_303 3 次失败）→ audit unsynced + WARN · 路径 A 仍完成 |
| 8 | TC-L103-L204-N02 | §3.6 + §11.1 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9 N2 | L1-10 尝试反写 metrics → 不可达（本 L2 无写接口暴露给 L1-10）|
| 9 | TC-L103-L204-N03 | §2.3 I-2 + §6.4 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9 N3 | completion_rate 按数量而非 effort 计算 → 契约测试检测口径不一致 |
| 10 | TC-L103-L204-N04 | §6.2 + §11.1 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9 N4 | 本 L2 直改 current-metrics.json 不走 IC-L2-04 → L2-02 一致性校验失败（集成测试）|
| 11 | TC-L103-L204-N05 | §1.7 + §11.1 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9 N5 | 本 L2 在 handler 内判 "应触发回退" → 拒绝编译（架构测试 · `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.5 禁止 7）|
| 12 | TC-L103-L204-N06 | §6.2 + §3.2 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9 N6 | 本 L2 过滤 wp_done 只打点关键路径 → 契约测试检测丢事件（全量消费违反）|
| 13 | TC-L103-L204-I01 | §5.1 + §4.3 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9 I1 | L1-04 wp_done → L2-04 打点 → L2-02 done → 并行度释放 → L2-03 下 tick 可取下一 WP |
| 14 | TC-L103-L204-I02 | §5.2 + §4.3 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9 I2 | L1-04 wp_failed → L2-04 打点 → L2-05 计数 +1 → counter=3 触发 BF-E-08（不经本 L2）|
| 15 | TC-L103-L204-I03 | §6.1 + §8.2 | `docs/2-prd/L1-03 WBS+WP 拓扑调度/prd.md` §11.9 I3 | 跨 session 重启：会话 A 完成 5/10 → kill → 会话 B system_resumed → L2-04 重建 completion_rate=50% → UI 正确展示 |
