...
本 L2 核心技术需求：**浅层 AST 解析 / 语言识别 / 符号图 / 框架检测**。调研以下高星开源项目评估采纳决策。

### 9.2 技术选型对比表

| 项目 | GitHub Stars | 语言 | 核心能力 | 优势 | 劣势 | L2-02 决策 |
|:---|:---|:---|:---|:---|:---|:---|
| **tree-sitter** | ~19k | C + bindings | 增量 AST 解析（40+ 语言） | 健壮·语法错误容忍·速度极快 | 需各语言 grammar 库·调用略复杂 | **采纳（可选）**：若需精确 import 提取则引入 tree-sitter-python/ts；当前版本用 regex 足够 |
| **Universal Ctags** | ~6k | C | 符号提取（Tags 格式） | 成熟·支持 100+ 语言·Shell 调用简单 | 只输出符号表·不给依赖图·无 Python API | **不采纳**：产物格式不符·依赖图需自建 |
| **jedi** | ~5.8k | Python | Python 静态分析·自动补全·引用跳转 | 完整符号图·类型推断 | 仅 Python·重量级·与 Claude 工具调用不匹配 | **不采纳**：深度分析留给 codebase-onboarding；本 L2 只做浅层 |
| **linguist** | ~11k | Ruby | GitHub 语言识别（文件扩展名 + heuristics） | 准确率高·规则完善 | Ruby 依赖·不适合嵌入 Python 服务 | **参考算法**：借鉴其 heuristics 策略自实现 LanguageClassifier |
| **enry** | ~2.2k | Go | linguist 的 Go 移植·支持 Python bindings | 速度快·Python 可调用 | Stars 较少·维护活跃度一般 | **备选**：若 LanguageClassifier 精度不足时引入 |
| **semgrep** | ~10k | Python/OCaml | 语义级 pattern matching（跨语言） | 规则丰富·精度高 | 重量级·安装复杂·适合 CI/CD | **不采纳**：PatternGrepScanner 用 regex 轻量实现足够；semgrep 留给 L1-04 Quality Gate |
| **ast-grep** | ~7k | Rust + WASM | 基于 tree-sitter 的结构化搜索 | 极快·支持多语言·grep-like 接口 | Rust/WASM·Python 集成尚不成熟 | **观察**：V2 版本可考虑替换 PatternGrepScanner |
| **pyflakes** | ~1.3k | Python | Python 静态分析（import 验证） | 轻量·准确 | 仅 Python | **不采纳**：与 import 提取目标重叠但方向不同 |

### 9.3 关键决策记录

**D-OSS-01 · tree-sitter 采纳策略**

当前版本（V1）：`ASTShallowParser` 用 regex 实现，无需引入 tree-sitter。
原因：
- 浅层 import 提取 regex 精度已足够（>95% 命中率 for Python/TS/Go）
- 避免 C 扩展编译依赖（部署简化）
- token 预算限制下，深度 AST 分析不在 scope

V2 升级路径：若 import 提取出现高频误报，引入 `tree-sitter-python` + `tree-sitter-typescript`，替换 `_parse_python_imports` / `_parse_ts_imports`。

**D-OSS-02 · linguist 算法借鉴**

`LanguageClassifier` 核心逻辑参考 linguist 的策略：
1. 文件扩展名映射（`.py` → Python · `.ts` → TypeScript 等）
2. Shebang 行识别（`#!/usr/bin/env python3`）
3. 配置文件特征（`pyproject.toml` → Python ecosystem）
4. 二进制文件过滤（magic bytes 检测）

不直接调用 linguist（Ruby 依赖）。

**D-OSS-03 · semgrep 边界**

PatternGrepScanner 的 regex 白名单 ≠ semgrep 替代。
- semgrep：语义级（理解 AST 上下文）
- PatternGrepScanner：基于行级 regex（轻量·快速）

两者不重叠：本 L2 用 regex·L1-04 Quality Gate 可选 semgrep 做深度扫描。
...
以下测试用例 ID 对应 `docs/3-2-Solution-TDD/L1/L2-02-tests.md`（待建）。
