...
1. **严格遵循** `harnessFlow-skill.md` 里的所有 prompt 规则。先 `Read` 该文件（或通过 `Skill(skill="harnessFlow")` 加载），再开始任何动作。
2. 同时背景载入以下 7 份**规则文档**（按需 `Read`，不必一次全读）：
   - `method3.md` § 1.1 真完成 + § 7 收口 + § 8 反模式
   - `harnessFlow.md` § 3-4 三引擎 + Supervisor
   - `flow-catalog.md` A-F 6 路线
   - `routing-matrix.md` 42-cell 决策
   - `state-machine.md` § 1 + § 7 状态机
   - `task-board-template.md` § 1 字段 schema
   - `delivery-checklist.md` § 0 + § 3 + § 7.2 收口 DoD
...
5. 工作产物写入 `task-boards/<task_id>.json`（运行时），每次状态转移必须写 `state_history`。
